"""Summarize P1-public v4 rule-eval results against prior public DPO runs."""
...
def write_spotcheck(path: Path, compare_dirs: dict[str, Path], failures: dict[str, dict[str, list[str]]]) -> None:
    focus_ids = sorted(set(failures["P1-public DPO v4"]["risk_fail"]) | set(failures["P1-public DPO v4"]["suitability_fail"]))
    ...
    lines = [
        "# P1-public v4 Failure Spot-check",
        "",
        f"- focus ids: `{focus_ids}`",
        f"- v4 risk fail ids: `{failures['P1-public DPO v4']['risk_fail']}`",
        f"- v4 suitability fail ids: `{failures['P1-public DPO v4']['suitability_fail']}`",
        "",
    ]
    for sample_id in focus_ids:
        lines.extend([f"## {sample_id}", ""])
        for name in ["P1-public DPO v2", "P1-public DPO v3", "P1-public DPO v4"]:
...
def main() -> None:
    ...
    parser.add_argument("--output_csv", default="outputs/eval/p1_public_v4/rule_summary.csv")
    parser.add_argument("--output_json", default="outputs/eval/p1_public_v4/failure_overlap.json")
    parser.add_argument("--output_md", default="reports/p1_public_v4_rule_summary.md")
    parser.add_argument("--spotcheck_md", default="reports/p1_public_v4_failure_spotcheck.md")
    ...
    overlap = {
        "failures": failures,
        "diffs": {
            "v2 vs v4": diff_failures(failures["P1-public DPO v2"], failures["P1-public DPO v4"], "v4"),
            "v3 vs v4": diff_failures(failures["P1-public DPO v3"], failures["P1-public DPO v4"], "v4"),
        },
    }
