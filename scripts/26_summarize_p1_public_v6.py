"""Summarize P1-public v6 rule-eval results against prior public DPO runs."""
...
def write_spotcheck(path: Path, compare_dirs: dict[str, Path], failures: dict[str, dict[str, list[str]]]) -> None:
    focus_ids = sorted(set(failures["P1-public DPO v6"]["risk_fail"]) | set(failures["P1-public DPO v6"]["suitability_fail"]))
    ...
    lines = [
        ...
        f"- v6 risk fail ids: `{failures['P1-public DPO v6']['risk_fail']}`",
        f"- v6 suitability fail ids: `{failures['P1-public DPO v6']['suitability_fail']}`",
        "",
    ]
    for sample_id in focus_ids:
        lines.extend([f"## {sample_id}", ""])
        for name in ["P1-public DPO v2", "P1-public DPO v3", "P1-public DPO v4", "P1-public DPO v5", "P1-public DPO v6"]:
...
def main() -> None:
    ...
    parser.add_argument("--v6_dir", default="outputs/eval/p1_public_v6")
    parser.add_argument("--output_csv", default="outputs/eval/p1_public_v6/rule_summary.csv")
    parser.add_argument("--output_json", default="outputs/eval/p1_public_v6/failure_overlap.json")
    parser.add_argument("--output_md", default="reports/p1_public_v6_rule_summary.md")
    parser.add_argument("--spotcheck_md", default="reports/p1_public_v6_failure_spotcheck.md")
    ...
    metric_paths = {
        ...
        "P1-public DPO v6": v6_dir / "dpo_rule_metrics.json",
    }
    detail_paths = {
        ...
        "P1-public DPO v6": v6_dir / "dpo_rule_details.jsonl",
    }
    ...
    overlap = {
        "failures": failures,
        "diffs": {
            "v2 vs v6": diff_failures(failures["P1-public DPO v2"], failures["P1-public DPO v6"], "v6"),
            "v3 vs v6": diff_failures(failures["P1-public DPO v3"], failures["P1-public DPO v6"], "v6"),
            "v5 vs v6": diff_failures(failures["P1-public DPO v5"], failures["P1-public DPO v6"], "v6"),
        },
    }
